...
- **Style C: 华为企业汇报**。白灰底、Huawei red、右上品牌位、目录红栏、流程箭头、数据条形图和人机协作/组织协议页,适合公司总结、工作计划、商务汇报和 AI 组织变革材料。
...
![Style C H01 封面](screenshots/huawei-slide-1.png)
![Style C H02 目录](screenshots/huawei-slide-2.png)
![Style C H08 流程箭头](screenshots/huawei-slide-4.png)
![Style C H26 比喻卡片](screenshots/huawei-slide-7.png)
![Style C H05 三卡片](screenshots/huawei-slide-11.png)
![Style C H10 结尾](screenshots/huawei-slide-15.png)
...
- 🖋 **三套视觉系统**:电子杂志风负责叙事,瑞士风负责事实表达,华为企业汇报风负责红灰公司总结和 AI 组织内容
...
- 🧾 **Style C 42 种企业汇报/商业计划/AI 组织版式**:Huawei Cover、Agenda、Section、Timeline、Data Bars、Process Arrow、Image Grid、KPI Dashboard、Map Locations、Photo Quote、Market Size、Pricing Table、SWOT、Gantt、Human Agent Loop、Org Protocol、Matching Engine、Before/After Case 等
- 🎨 **主题色预设**:Style A 5 套电子墨水主题,Style B 4 套瑞士高饱和锚点色,Style C 3 套 Huawei corporate 红灰主题
...
| 公司总结 / 工作计划 / 商务汇报 / AI 组织变革 | 用 Style C 华为企业汇报风,优先使用 H01-H42 企业/商业计划/AI 组织版式 |
...
- **更容易做质量控制**:瑞士风和华为企业汇报风都有脚本校验版式、图片槽位/品牌位、标题对齐和危险 SVG。
...
1. **选择风格** — Style A 电子杂志风,Style B 瑞士国际主义,或 Style C 华为企业汇报
2. **需求澄清** — 7 问清单:风格、受众、时长、素材、图片/截图需求、主题色、硬约束
3. **拷贝模板** — Style A 用 `assets/template.html`,Style B 用 `assets/template-swiss.html`,Style C 用 `assets/template-huawei.html`
4. **填充内容** — 先做主题节奏表,再从对应 layout 骨架里挑、粘、改文案
5. **可选配图** — 在 Codex 中询问是否用 GPT-Image 2.0 / GPT-M 2.0 生成配图,再按页面比例插入
6. **自检** — 对照 `references/checklist.md`,P0 级问题必须全过；瑞士风和华为企业汇报风还要运行对应校验器
7. **预览** — 浏览器直接打开
...
华为企业汇报风是红灰企业报告系统,不只是"红色主题"。它适合公司总结、工作计划、商业计划,也适合 AI 组织、人机协作和运营机制类材料。

- **42 个具名版式**:正文页优先从 `H01` 到 `H42` 中选择,覆盖封面/目录/章节、正文、数据、流程、证据、商业计划,以及 H37-H42 的 AI 组织内容页
- **新增 AI 组织内容形状**:H37 Human Agent Loop、H38 On/In Governance Split、H39 Org Protocol Board、H40 Matching Engine、H41 Metaphor Mapping、H42 Before/After Case
- **同层子容器等尺寸**:同级卡片、步骤、角色、指标、触发条件尽量共宽共高；优先用 `.grid-*`、`.same-size`、`.equal-children`、`.mini-card`
- **品牌与页码固定**:右上统一使用 `.brand-lockup`；页码由模板自动生成在左下角 `.page-no`；不要在 `.chrome` 里手写页码
- **红灰企业感**:普通正文页用红色做锚点、编号、箭头和数据重点；大面积红只留给封面、目录、章节和结尾
- **强校验**:模板规则和 deck 规则分开校验,避免只扩版式、不扩文档或校验器
...
按所选风格从对应主题文件里选一套——**不允许自定义 hex 值**,保护美学比给自由更重要。
...
华为企业汇报风从 `references/themes-huawei.md` 里选一套,同样**不允许自定义 hex 值**。

| 主题 | 核心色与适合场景 |
|------|------------------|
| **Classic Red** | `#c7000b`。默认华为红灰企业汇报,适合公司总结、工作计划、商务路演。 |
| **Deep Executive** | `#b00010`。更稳重,适合管理层、战略、商业计划。 |
| **Cloud Tech** | `#d20a2e`。更现代偏冷灰,适合 ICT、云、AI、研发总结。 |

如果用户说"华为 PPT"或"企业汇报红灰模板"但没有指定主题,默认推荐 Classic Red。
...
9. **华为企业汇报风必须守红灰企业感** — Style C 只用红/黑/灰/白做主视觉,普通正文页不要滥用满屏红色
10. **华为同层容器尽量等尺寸** — Style C 的同级卡片、步骤、角色和指标应尽量保持相同宽高,优先用模板已有等尺寸工具类
...
**我能加自己的版式吗?**
可以。Style A 可以在 `references/layouts.md` 里扩展；Style B 更严格,需要同步更新 `template-swiss.html`、`layouts-swiss.md`、`swiss-layout-lock.md` 和校验器；Style C 需要同步更新 `template-huawei.html`、`references/layouts-huawei.md` 和 `scripts/validate-huawei-*` 校验器。
...
- 在 `template-huawei.html` 里补类时,同步更新 `layouts-huawei.md` 和 `scripts/validate-huawei-*`
- 瑞士风新增规则后,同步更新 `scripts/validate-swiss-deck.mjs`
- 华为企业汇报新增规则后,同时更新 README、`SKILL.md`、`references/layouts-huawei.md` 和校验器
- 把踩过的坑写到 `checklist.md` 对应的 P0 / P1 / P2 / P3 级别
- 新主题色进对应风格的 `themes*.md` 并给出适合的场景
